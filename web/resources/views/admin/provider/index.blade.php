    ...
    <div class="box bo-co float">
      <div class="box-body">
        <div class="col-xs-12 from-lex" >
          <form action="{{ route('provider/search') }}" method="POST" class="from-bpox-in">
            @csrf
          ...
          <a href="{{route('provider/add')}}" style="float: right;margin-top: -23px">
                 ...
                 <!--  <td>{{$data->address}}</td> -->
                  ...
                  <!--  <td>
                    ...
                    @if(!empty($data->image))
                      ...
                      <img src="{{asset('public/image/provider_profile/'.$data->image)}}" class="img-circle" height="50px" width="50px"></a>
                    @else
                      <img src="{{asset('public/dummy.jpg')}}" height="50px" width="50px" class="img-circle">
                    @endif
                  </td>
                  <td>
                    @if($data->status == '0')
                      <a class="btn btn-success btn-sm" onclick="return confirm('Are you sure, you want to de-active this provider?')" href="{{ route('provider/status',[$data->id,$data->status])}}" title="Deactive User">
                        <i class="fa fa-lock"></i> Active
                      </a>
                    @endif  
                    @if($data->status == '1')
                      <a class="btn btn-warning btn-sm" onclick="return confirm('Are you sure, you want to Active this provider?');" href="{{ route('provider/status',[$data->id,$data->status])}}" title="Active User">
                  ...
                  <td style="width:70px;">
                    <center>
                      <form method="POST" action="{{route('provider/delete',[$data->id,$page])}}" class="form-inline">
                        <a href="{{route('provider/view',base64_encode($data->id))}}" class="btn btn-warning btn-sm">
                          <i class="glyphicon glyphicon-eye-open"></i>
                        </a>
                        <a href="{{route('provider/edit',[base64_encode($data->id),$page])}}" class="btn btn-primary btn-sm">
